/**
 * version:1.2.2
 * update:2016年8月9日 08:47:11
 * tip:
 * 1、采用类似jquery的非new实例化模式
...
var Junb=(function () {
    ...
    var Junb = function () {
        ...
        self.extend = function () {
            if (arguments[0] == "pubPre") {
            ...
            } else if (arguments[0] == "pubBefore") {
                self.beforeRender=arguments[1];//渲染之前，在preFn之前，即使sync为false，都需要执行
            } else if (arguments[0] == "pubTemp") {//公用模板
                var temp_id=arguments[1];
                self.temps[temp_id]={
                    0:(function(){
                ...
                pages[o.id] = {
                    ...
                    show: function (cateHash) {
                        var t = this;
                        Junb.addClass(t[0], "on");
                        self.beforeRender&&self.beforeRender();
                        if (self.sync) {
                            //执行渲染前函数
                            if (self.preFn && self.preFn.length !== 0) {
                                for (var i = 0, len = self.preFn.length; i < len; i++) {
