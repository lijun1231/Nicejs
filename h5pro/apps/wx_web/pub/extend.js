...
!function($){
    $.extend({
        ...
        //delay:重发时间
        //check：检验，若返回true则继续执行，若返回false则停止执行。若没有此值则忽略
        //callback:回调函数
        ...
        VerifiCode: function (o) {
            ...
            function countTime() {
                if(!o.check||o.check()===false){return false;}
                if(o.fnWrong){
        ...
        bottomUp: function (o) {
            ...
            if (typeof (o) == "string") {
                ...
                if (o.layer == true) {//添加遮罩层
                    var $layer=$(document).find(".model-pop-layer");
                    if ($layer.length == 0) {
                        $layer = $("<div class=\"model-pop-layer\" style=\"display:none;\"></div>").appendTo('body');
                    }
                    //单击暗层是否关闭弹窗
                    if(o.layerClose){
                        $layer.on(type,function(){
                ...
                //是否含关闭按钮
                if (o.closeBt) {
                    var $closeBt=$("<div class=\"model-pop-closeBt\"></div>");
                    if($closeBt.length===0){
                        $closeBt=$("<div class=\"model-pop-closeBt\"></div>").appendTo($(this));
                    }
                    $closeBt[0].addEventListener(type,function () {
                        $(othis).bottomUp("close");
                    },false);
                }
                //按钮组
                if(o.buttons&&o.buttons.length>0){
                    ...
                    if($myBts.length===0){
                        $myBts=$("<div class=\"model-pop-bts\"></div>").appendTo($(this));
                    }
                    var strButtons = "";
